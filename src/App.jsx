...
const tabs = [
  ...
  { id: "taches",   label: "✅", name: "Taches"   },
  { id: "epicerie", label: "🛒", name: "Epicerie" },
...
export default function FamilleApp() {
      ...
      if (val) {
        setData(prev => {
          if (prev && val.lastUpdatedBy && val.lastUpdatedBy !== parent?.id && val.lastUpdatedAt !== prev?.lastUpdatedAt) {
            ...
            showToast(`${who.emoji} ${who.label} a mis a jour l'app !`);
          }
          return val;
        });
      } else {
  ...
  return (
    <div style={pgStyle}>
      ...
      {toast && (
        <div style={{ position:"fixed", top:16, left:"50%", transform:"translateX(-50%)", background:"#2D2D2D", color:"#fff", borderRadius:16, padding:"12px 20px", fontSize:14, fontWeight:700, zIndex:999, boxShadow:"0 4px 20px rgba(0,0,0,0.25)", whiteSpace:"nowrap" }}>
          {toast}
        </div>
      )}
      ...
      <div style={{ background:parent.color, padding:"24px 20px 18px", borderRadius:"0 0 28px 28px", boxShadow:`0 4px 20px ${parent.color}55` }}>
        ...
        {data.lastUpdatedBy && data.lastUpdatedBy !== parent.id && (
          <div style={{ marginTop:10, background:"rgba(255,255,255,0.18)", borderRadius:12, padding:"8px 12px", display:"flex", alignItems:"center", gap:8 }}>
            ...
            <span style={{ fontSize:12, color:"rgba(255,255,255,0.95)", fontWeight:700 }}>{otherParent?.label} a modifie {timeAgo(data.lastUpdatedAt)}</span>
          </div>
        )}
        <div style={{ display:"flex", gap:8, marginTop:14 }}>
          {[{ label:"taches", val:tachesLeft, icon:"✅" }, { label:"epicerie", val:epicerieLeft, icon:"🛒" }, { label:"evenements", val:(data.agenda||[]).length, icon:"📅" }].map((s,i) => (
      ...
      <div style={{ padding:"18px 16px 0" }}>
        {activeTab === "accueil" && (
          <div className="fade-in">
            ...
            <ST style={{ marginTop:20 }}>A faire</ST>
        ...
        {activeTab === "taches" && (
          <div className="fade-in">
            <ST>A faire ({tachesLeft})</ST>
            {(data.taches||[]).filter(t => !t.done).map(t => (
              <TacheItem key={t.id} t={t} onToggle={toggleTache} onDelete={deleteTache} color={parent.color} />
            ))}
            {(data.taches||[]).some(t => t.done) && <><ST style={{ marginTop:20, opacity:0.5 }}>Completees</ST>{(data.taches||[]).filter(t => t.done).map(t => <TacheItem key={t.id} t={t} onToggle={toggleTache} onDelete={deleteTache} color={parent.color} done />)}</>}
            <Card>
        ...
        {activeTab === "epicerie" && (
          <div className="fade-in">
            ...
            <Card>
              ...
              <div style={{ display:"flex", gap:8, marginTop:10 }}>
                <input value={newQty} onChange={e => setNewQty(e.target.value)} placeholder="Quantite" style={{ ...inp, flex:0.6 }} />
          ...
          <div className="fade-in">
            ...
            {(data.agenda||[]).map(ev => (
              <div key={ev.id} style={{ background:"#fff", borderRadius:16, padding:"13px 16px", marginBottom:10, display:"flex", gap:14, alignItems:"center", boxShadow:"0 2px 10px rgba(0,0,0,0.06)" }}>
                ...
                <button onClick={() => deleteEvent(ev.id)} style={del}>X</button>
              </div>
            ))}
            <Card>
              ...
              <input value={newEvent} onChange={e => setNewEvent(e.target.value)} onKeyDown={e => e.key==="Enter" && addEvent()} placeholder="Nom de l'evenement..." style={inp} />
            ...
            <div style={{ display:"flex", flexWrap:"wrap", gap:12 }}>
              {(data.notes||[]).map(n => (
                <div key={n.id} style={{ background:n.color, borderRadius:18, padding:14, flex:"1 1 44%", minWidth:150, position:"relative", boxShadow:"0 3px 10px rgba(0,0,0,0.08)" }}>
                  ...
                  <button onClick={() => deleteNote(n.id)} style={{ position:"absolute", top:8, right:8, background:"rgba(0,0,0,0.1)", border:"none", borderRadius:"50%", width:22, height:22, cursor:"pointer", fontSize:12, color:"#555" }}>X</button>
                </div>
              ))}
              <div style={{ flex:"1 1 100%", background:"#fff", borderRadius:18, padding:16, boxShadow:"0 2px 10px rgba(0,0,0,0.06)", marginTop:4 }}>
                <textarea value={newNote} onChange={e => setNewNote(e.target.value)} placeholder="Ecrire une note..." rows={3} style={{ ...inp, width:"100%", resize:"none", lineHeight:1.5 }} />
                <button onClick={addNote} style={{ ...btnSt(parent.color), width:"100%", marginTop:8 }}>Epingler</button>
              </div>
            </div>
          </div>
        )}
...
function TacheItem({ t, onToggle, onDelete, done, color }) {
  return (
    <div style={{ background:"#fff", borderRadius:16, padding:"12px 14px", marginBottom:10, display:"flex", alignItems:"center", gap:12, boxShadow:"0 2px 10px rgba(0,0,0,0.06)", opacity:done?0.5:1 }}>
      ...
      <button onClick={() => onDelete(t.id)} style={del}>X</button>
    </div>
  );
}
  ...
  return (
    <div style={{ background:"#fff", borderRadius:16, padding:"12px 14px", marginBottom:10, display:"flex", alignItems:"center", gap:12, boxShadow:"0 2px 10px rgba(0,0,0,0.06)", opacity:done?0.5:1 }}>
      ...
      <div style={{ flex:1 }}>
        ...
        <div style={{ fontSize:11, color:"#9A9A9A", marginTop:2, fontWeight:600 }}>Qte : {e.qty} {parentInfo(e.by).emoji}</div>
      </div>
      <button onClick={() => onDelete(e.id)} style={del}>X</button>
    </div>
  );
}
